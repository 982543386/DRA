function [resp, util, rejt, time] = DLA(data)
    ...
    breakpoint = 0;   % breakpoint of employing random scheduling policy to collect trianing data
    ...
    for i=1:timeslots
        ...
        for u_id = Users
            tic;
            if R_c == Record_count
                R_f = 1; breakpoint = i; tt=toc; break;
            end        
            R_c = R_c + 1;
    ...
    [beta, res, flag] = linprog(b', -AA', -C, [],[],lb,[], options);
    beta = beta(1:m);
    ...
    % totalResp   = 0;
    updateCount = 0;
    updateFlag  = Record_count * 2;

    % TODO:update C_max value when update beta
    for i=breakpoint:timeslots
        Users = U_arriavaltime{i};
        for u_id = Users
            
            updateCount = updateCount + 1;
            if updateCount > updateFlag
            ...
            tic; % scheduling policy training and task offloading decision-making process are asynchronous, thus the training time are not recorded
            t_ES_ids     = U_avaes{u_id};
            t_ES_avaids  = [];
            t_U_tasksize = U_tasksize(u_id);
            for j=1:length(t_ES_ids)
            ...
            if isempty(t_ES_avaids)
                ...
                if scount == length(t_ES_score)
                    ...
                    C(updateCount) = sc;
                    A(e_id, updateCount) = t_U_tasksize;
                end
            end
            time(end+1) = toc;
        end
